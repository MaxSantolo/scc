...
/*        tbody:hover td {
    ...
    <h1>BOLLE/FATTURE FINO AL <?php echo date('Y')-1 ?></h1>
...
if (isset($_POST['annulla_cerca'])) {
            ...
            $bolle_fatture_intero = $conn_prod_cespiti->query("SELECT * FROM bolle_fatture WHERE id NOT LIKE '".DATE('Y')."-%' ORDER BY CONVERT(id, DECIMAL)");
        }
...
if (isset($_POST['cerca_bollfatt'])) { 
    ...
    $bolle_fatture_intero = $conn_prod_cespiti->query("SELECT * FROM bolle_fatture WHERE (fattura_data BETWEEN '".$dal."' AND '".$al."') AND fornitore LIKE '%".$fornitore."%' AND nr_fattura LIKE '%".$fattura."%' AND id NOT LIKE '".DATE('Y')."-%'ORDER BY id"); }
  
    else { $bolle_fatture_intero = $conn_prod_cespiti->query("SELECT * FROM bolle_fatture WHERE id NOT LIKE '".DATE('Y')."-%' ORDER BY CONVERT(id, DECIMAL)"); }
...
//                        . "<td style='text-align: center'><A HREF='".$row["scansione_fattura"]."' TARGET='_blank'><IMG SRC=\"immagini/visualizza.png\" width=\"25\" TITLE=\"MOSTRA FATTURA\"></A></td>
                        ."<td>".$row["note"]."</td>"
                        . "<td width='25'> <a href='dettaglio_bolla_fattura.php?id=".$row["id"]."'><img src='immagini/dettagli.png' width='25' TITLE='DETTAGLI'></a></td><td width='25'> <a href='tech/funzioni.php?fx=eliminabolfat_old&id_f=".$row["id"]."' onclick=\"return confirm('Vuoi veramente cancellare questa bolla? Tieni presente che se la bolla ha dei cespiti associati non potrà essere eliminata.')\"><img src='immagini/elimina.png' width='25' TITLE='ELIMINA'></a></td>";
		}
		echo "</table>";
